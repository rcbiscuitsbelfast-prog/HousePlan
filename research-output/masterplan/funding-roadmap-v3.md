...
```mermaid
flowchart LR
  A[Steering group] --> B[Eligible legal vehicle]
  B --> C[AHF viability/development funding]
  C --> D[Surveys and options appraisal]
  D --> E[BCC/HED pre-app]
  E --> F[NLHF project enquiry/EOI]
  F --> G[Match funding package]
  G --> H[Capital application]
  H --> I[Delivery and claims]
```
